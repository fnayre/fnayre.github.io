...
// I'm switching to an interface definition for better DX
...
// This should work polymorhically for any Functor
const Identity = {
  map(f, x) {
    return f(x);
  }
...
function compose<S, T, A, B, X, Y>(
  ...
  return {
    $applyOptic(F, f, s) {
      return parent.$applyOptic(
        F as any,
        a => child.$applyOptic(F as any, f, a),
        s
      );
    }
  };
...
function over<S, T, A, B>(l: Lens<S, T, A, B>, f: Fn<A, B>, s: S): T {
  return l.$applyOptic(Identity as Functor<B, T, B, T>, f, s);
}

function prop<S>() {
  return <K extends keyof S>(k: K): SimpleLens<S, S[K]> => {
    return lens(s => s[k], (a, s) => Object.assign({}, s, { [k]: a }));
  };
}
...
const address = prop<Person>()("address");
const addresses = prop<Person>()("addresses");
const num = prop<Address>()("num");
...
If you're familiar with the usual Applicative definitionfrom Haskell, the definition of `combine` may be a little surprising (i,e, I 'should' define something like `apply: (F<A -> B>, F<A>) -> F<B>`). There are 2 reasons for the above choice:
